...
from utils import set_quantum_flag, get_quantum_flag

def show_settings_page(mongo_db, user_id):
    """Display the settings page with analytics, preferences, and quantum feature toggles."""
            ...
            for idx, query in enumerate(analytics["recent_queries"]):
                ...
                with col2:
                    ...
                    if isinstance(ts, datetime):
                        ...
                        st.caption(f"{ts}")
                st.caption(f"Response Time: {query.get('response_time', 0):.2f}s")
...
def show_preferences(mongo_db, user_id):
    """Display user preferences settings with quantum feature toggles."""
    ...
    with q1:
        set_quantum_flag(
            "enable_quantum_embeddings",
            st.checkbox("Quantum Embeddings", value=get_quantum_flag("enable_quantum_embeddings"))
        )
        set_quantum_flag(
            "enable_quantum_rerank",
            st.checkbox("Quantum Re-ranking", value=get_quantum_flag("enable_quantum_rerank"))
        )
    with q2:
        set_quantum_flag(
            "enable_quantum_context_selection",
            st.checkbox("Quantum Context Selection", value=get_quantum_flag("enable_quantum_context_selection"))
        )
        set_quantum_flag(
            "enable_quantum_inspired_system",
            st.checkbox("Quantum-Inspired System", value=get_quantum_flag("enable_quantum_inspired_system"))
        )
    with q3:
        set_quantum_flag(
            "enable_quantum_secure_storage",
            st.checkbox("Quantum-Safe Storage", value=get_quantum_flag("enable_quantum_secure_storage"))
        )
...
state-of-the-art Retrieval Augmented Generation (RAG) technology to help organize,
search, and extract insights from documents.

Built with GPU acceleration and a quantum-ready architecture:
- Quantum embeddings
- Quantum-optimized context selection
- Quantum-safe storage
- Quantum-assisted re-ranking
- Quantum-inspired acceleration paths
""")
...
- Custom Document Naming: Organize documents with preferred names
- Document Viewer: Read documents without leaving the application
""")
