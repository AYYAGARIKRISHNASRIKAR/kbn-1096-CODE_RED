...
from utils import get_quantum_flag
...
def show_chat_page(mongo_db, user_id, rag_system=EnhancedRAG):
    ...
    # Quantum module badges (read-only indicator)
    q_badges = []
    if get_quantum_flag("enable_quantum_embeddings"): q_badges.append("Embeddings")
    if get_quantum_flag("enable_quantum_context_selection"): q_badges.append("Context")
    if get_quantum_flag("enable_quantum_rerank"): q_badges.append("Re-rank")
    if get_quantum_flag("enable_quantum_secure_storage"): q_badges.append("Secure")
    if get_quantum_flag("enable_quantum_inspired_system"): q_badges.append("Q-Inspired")
    if q_badges:
        st.caption("Quantum modules active: " + ", ".join(q_badges))
    ...
    with st.sidebar:
        ...
        llm_model = st.selectbox(
            ...
            options=[
                ...
                "phi3.5:3.8b", "dolphin-phi:latest",
                "samantha-mistral:latest", "dolphin-mistral:latest",
            ],
            index=0,
            key="chat_llm_model"
        )
    ...
    if uploaded_files and st.button("Process Files"):
        with st.spinner("Processing files..."):
            ...
            if success and hasattr(st.session_state.rag, "processing_times"):
                metrics = {
                    "documents_processed": getattr(st.session_state.rag, "documents_processed", 0),
                    "index_building_time": st.session_state.rag.processing_times.get("index_building", 0.0),
                    "total_processing_time": st.session_state.rag.processing_times.get("total_time", 0.0),
                    "memory_used_gb": st.session_state.rag.processing_times.get("memory_used_gb", 0.0),
                    "device": st.session_state.rag.device
                }
                    ...
                    try:
                        with st.spinner(f"Processing with {st.session_state.rag_mode} mode..."):
                            ...
                            if isinstance(response, str) and "Please upload and process documents first" in response:
                                potential_domains = getattr(st.session_state.rag, "detect_query_domain", lambda x: [])(user_input)
                                ...
                                if isinstance(response, dict):
                                    ...
                                    if "mode" in response:
                                        ...
                                        mode_icons = {"direct_retrieval": "📄", "enhanced_rag": "🔄", "hybrid": "🌐"}
                                        icon = mode_icons.get(mode_name, "ℹ")
                                        st.caption(f"{icon} Answer mode: {mode_name}")
                                        ...
                                        if "sources" in response and response["sources"]:
                                            with st.expander("📄 View Sources"):
                                                for j, source in enumerate(response["sources"]):
                                                    if source.get("file_type") == "web":
                                                        st.markdown(f"*Source {j+1}: 🌐 {source['source']}*")
                                                    else:
                                                        st.markdown(f"*Source {j+1}: 📄 {source['source']}*")
                                                    st.text(source["content"])
                                                    st.divider()
