...
def main():
    ...
    # Keep your styling hook
    set_page_style()

    # Initialize all session variables (includes quantum flags; no UI change)
    init_session_state()

    # Ensure Mongo client exists once per session
    ...
    # Unauthenticated landing with your exact HTML
    if not logged_in and st.session_state.user is None:
        ...
        # Preserve your page routing between login/signup
    ...
    with st.sidebar:
        ...
        # Functional: update page state and rerun only if changed
    ...
    # Page routing (kept identical semantics)
    if st.session_state.page == "notebooks":
        ...
        # Pass EnhancedRAG class (quantum-ready) without changing UI
        ...
        # Chat uses the quantum-ready EnhancedRAG but UI remains yours
        show_chat_page(
            st.session_state.mongo_db, 
            st.session_state.user['user_id'],
            EnhancedRAG
...
if __name__ == "__main__":
    # Cleanup temp files on exit (functional improvement; no UI impact)
